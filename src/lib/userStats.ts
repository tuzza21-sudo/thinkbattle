import { getDebateRecords } from './history';
import type { Player } from '../types';

export interface UserStats {
  xp: number;
  level: number;
  league: '초급' | '중급' | '고급' | '마스터';
  rankBadge: string;
}

export const calculateUserStats = async (userId: string): Promise<UserStats> => {
  const records = await getDebateRecords(userId);
  const totalXp = records.reduce((sum, record) => sum + (record.report?.xpEarned || 0), 0);

  const uncappedLevel = totalXp < 2000
    ? Math.floor(totalXp / 200)
    : totalXp < 8000
      ? 10 + Math.floor((totalXp - 2000) / 600)
      : totalXp < 20000
        ? 20 + Math.floor((totalXp - 8000) / 1200)
        : 30;
  const level = Math.min(30, uncappedLevel);
  const { league, rankBadge }: Pick<UserStats, 'league' | 'rankBadge'> = level >= 30
    ? { league: '마스터', rankBadge: '다이아몬드' }
    : level >= 20
      ? { league: '고급', rankBadge: '플래티넘' }
      : level >= 10
        ? { league: '중급', rankBadge: '골드' }
        : level >= 5
          ? { league: '초급', rankBadge: '실버' }
          : { league: '초급', rankBadge: '브론즈' };

  return {
    xp: totalXp,
    level,
    league,
    rankBadge
  };
};

export const getPlayerFromStats = async (userId: string, nickname: string): Promise<Player> => {
  const stats = await calculateUserStats(userId);
  return {
    id: userId,
    name: nickname,
    avatar: `https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(nickname)}`,
    level: stats.level,
    rankBadge: stats.rankBadge,
    score: stats.xp, // using XP as score
    streak: 0,
    isAi: false,
    league: stats.league === '마스터' ? '고급' : stats.league,
  };
};
