...
const privacySections = [
  ...
  ['음성 토론 처리', '음성방에서는 마이크 음성이 LiveKit을 통해 다른 참가자에게 실시간 전달됩니다. 발언 종료 시 브라우저의 임시 녹음이 전사를 위해 Google Gemini API로 전송됩니다. ThinkFit 서버는 원본 음성 파일을 보관하지 않으며, 생성된 전사문은 토론 진행과 복습 기록을 위해 Supabase에 저장됩니다.'],
...
const termsSections = [
  ...
  ['사람 간 토론 규칙', '참가자는 상대방의 동의와 존엄을 존중해야 하며 모욕, 협박, 혐오, 개인정보 노출, 불법 콘텐츠 공유를 해서는 안 됩니다. 음성방 입장 전 실시간 음성 전달과 전사문 저장 방식을 확인해야 합니다.'],
...
const englishPrivacy = [
  ...
  ['Voice debates', 'Live microphone audio is delivered to other participants through LiveKit. A temporary browser recording is sent to Google Gemini for transcription after a speech. ThinkFit does not retain the original audio file; the resulting transcript is stored in Supabase for the debate and review record.'],
