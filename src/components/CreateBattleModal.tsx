...
export const CreateBattleModal = ({
  ...
  const handleStart = async () => {
    ...
    if (battleMode === 'pvp' && !topicDescription.trim()) {
      setSubmitError('사람과 토론할 방에는 참가자가 읽을 상세 배경 설명이 필요합니다.');
      return;
    }
    ...
    <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="create-debate-title">
      <div className="modal-content debate-setup-modal">
        ...
        <div className="debate-setup-body">
          ...
          <section className="setup-section">
            <div className="setup-section-title"><span><BookOpen size={15} /></span><strong>토론 주제 상세 배경</strong></div>
            <textarea
              className="input-textarea"
              placeholder="참가자가 토론 전에 알아야 할 배경, 핵심 맥락, 다룰 범위와 제약을 입력하세요."
              value={topicDescription}
              maxLength={2000}
              onChange={event => setTopicDescription(event.target.value)}
              style={{ minHeight: 110 }}
            />
            <small className="setup-helper">{battleMode === 'pvp' ? '필수 · ' : ''}대기실과 토론 화면에서 모든 참가자에게 표시됩니다. ({topicDescription.length}/2000)</small>
          </section>
        ...
        <div className="debate-modal-footer">
          ...
          <button type="button" className="btn btn-primary" disabled={!topic.trim() || (battleMode === 'pvp' && !topicDescription.trim()) || isSubmitting} onClick={() => void handleStart()}>
